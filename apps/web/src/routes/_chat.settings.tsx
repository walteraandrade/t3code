...
import { type CustomQuickAction, DEFAULT_UNIFIED_SETTINGS } from "@t3tools/contracts/settings";
...
function SettingsRouteView() {
  ...
  const addQuickAction = useCallback(() => {
    ...
    if (!label) {
      setQuickActionError("Enter a label.");
      return;
    }
    if (!prompt) {
      setQuickActionError("Enter a prompt.");
      return;
    }
    if (label.length > MAX_QUICK_ACTION_LABEL_LENGTH) {
      setQuickActionError(`Labels must be ${MAX_QUICK_ACTION_LABEL_LENGTH} characters or less.`);
      return;
    }
    if (prompt.length > MAX_QUICK_ACTION_PROMPT_LENGTH) {
      setQuickActionError(`Prompts must be ${MAX_QUICK_ACTION_PROMPT_LENGTH} characters or less.`);
      return;
    }
    if (settings.customQuickActions.some((action) => action.label === label)) {
      setQuickActionError("That quick action label is already saved.");
      return;
    }
    if (settings.customQuickActions.length >= MAX_CUSTOM_QUICK_ACTIONS) {
      setQuickActionError(`You can save up to ${MAX_CUSTOM_QUICK_ACTIONS} quick actions.`);
      return;
    }

    updateSettings({
      customQuickActions: normalizeCustomQuickActions([
        ...settings.customQuickActions,
        {
          id: randomUUID(),
          label,
          prompt,
  ...
  const removeQuickAction = useCallback(
    (id: string) => {
      updateSettings({
        customQuickActions: settings.customQuickActions.filter((action) => action.id !== id),
      });
      setQuickActionError(null);
    ...
    [editingQuickActionId, settings.customQuickActions, updateSettings],
  );
  ...
  const saveEditingQuickAction = useCallback(() => {
    ...
    if (
      settings.customQuickActions.some(
        (action) => action.id !== editingQuickActionId && action.label === label,
      )
    ) {
      setQuickActionError("That quick action label is already saved.");
      return;
    ...
    updateSettings({
      customQuickActions: normalizeCustomQuickActions(
        settings.customQuickActions.map((action) =>
          action.id === editingQuickActionId
            ? {
                ...action,
                label,
                prompt,
                editBeforeSend: editingEditBeforeSend,
              }
            : action,
        ),
      ),
