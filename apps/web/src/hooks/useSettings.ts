...
import { normalizeCustomQuickActions } from "~/gitQuickActions";
    ...
    (patch: Partial<UnifiedSettings>) => {
      ...
      if (Object.keys(clientPatch).length > 0) {
        setClientSettings((prev) => {
          const next = { ...prev, ...clientPatch };
          return {
            ...next,
            customQuickActions: normalizeCustomQuickActions(next.customQuickActions),
          };
        });
      }
    },
...
export function buildLegacyServerSettingsMigrationPatch(legacySettings: Record<string, unknown>) {
  ...
  if (Schema.is(ModelSelection)(legacySettings.textGenerationModelSelection)) {
    ...
    const model = legacySettings.textGenerationModel.trim();
    if (model.length > 0) {
      patch.textGenerationModelSelection = {
        provider: model.startsWith("claude") ? "claudeAgent" : "codex",
        model,
      };
    }
  }
...
export function buildLegacyClientSettingsMigrationPatch(
  ...
  if (Array.isArray(legacySettings.customQuickActions)) {
    patch.customQuickActions = normalizeCustomQuickActions(
      legacySettings.customQuickActions.filter(Schema.is(CustomQuickActionSchema)),
    );
  }
